...
export const startLocationScrapping = async (page: Page): Promise<PackageInfo[]> => {
    return await page.evaluate(() => {
        ...
        packageElements.forEach((packageElement) => {
            ...
            packageInfo.name =
                (packageElement.querySelector(".package-name a") as HTMLElement).textContent || "";
            ...
            const inclusionElement = packageElement.querySelector(".package-inclusions");

            const inclusionItems = Array.from(
                inclusionElement?.querySelectorAll("li") || [])
                .map(
                    (item) =>
